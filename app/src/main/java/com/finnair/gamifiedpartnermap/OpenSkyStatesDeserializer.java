...
public class OpenSkyStatesDeserializer extends StdDeserializer<OpenSkyStates> {
    ...
    private Collection<OpenSkyStateVector> deserializeStates(JsonParser jp) throws IOException {
        ArrayList<OpenSkyStateVector> result = new ArrayList<>();
        ...
        for (JsonToken next = jp.nextToken(); next != null && next != JsonToken.END_ARRAY; next = jp.nextToken()) {
            ...
            OpenSkyStateVector sv = new OpenSkyStateVector(icao24);
            ...
            sv.setPositionSource(OpenSkyStateVector.PositionSource.values()[jp.nextIntValue(0)]);
